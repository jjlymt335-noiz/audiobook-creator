...
export async function segmentDialogueWithGemini(text: string, language: string): Promise<SegmentedDialogue> {
  ...
  const prompt = `You are analyzing an audiobook transcript that doesn't have clear speaker markers.
...
3. What role each speaker plays (narrator, character, etc.)
...
- For audiobooks, typically has 1 narrator, but may have multiple character voices
...
Important:
- speaker_name should be simple: "A", "B", "C", etc. or "Narrator", "Character A", "Character B"
  ...
  try {
    ...
    const segments = paragraphs.map((para, idx) => {
      ...
      return {
        ...
        speaker_role: idx % 2 === 0 ? 'Narrator' : 'Character A',
        text: para.trim(),
