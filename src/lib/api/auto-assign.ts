...
function parseDialogueFormat(
  ...
  const speakers = Array.from(speakerMap.entries()).map(([baseName, data], idx) => {
    ...
    const cleanSpeakerName = data.role || (idx === 0 ? 'Narrator' : `Character ${idx + 1}`);
    const speakerId = `speaker_${idx + 1}`;

    return {
      speaker_id: speakerId,
      speaker_name: cleanSpeakerName,
      character_description: data.role || (idx === 0 ? 'Primary narrator' : `Character ${idx + 1}`),
...
function parseByParagraphs(text: string, language: string): AutoAssignVoicesResponse {
  ...
  for (let i = 0; i < speakerCount; i++) {
    ...
    speakers.push({
      ...
      speaker_name: i === 0 ? 'Narrator' : `Character ${i + 1}`,
      character_description: i === 0 ? 'Primary narrator' : `Character ${i + 1}`,
      segments,
      matched_voice: {
  ...
  if (matches.length < 2) {
    ...
    try {
      ...
      for (let idx = 0; idx < speakerArray.length; idx++) {
        ...
        const cleanSpeakerName = speakerData.role || (idx === 0 ? 'Narrator' : `Character ${idx + 1}`);
        const speakerId = `speaker_${idx + 1}`;
